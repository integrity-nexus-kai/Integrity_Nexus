# Fundamental Candidate Admission Criteria

## Purpose

This document defines the admission rules by which TIG-E classifies candidates before they may enter a fundamental candidate pool.

These rules are universal and domain-neutral. Domain packs may define domain-specific candidate types, but they may not bypass the admission logic.

## Core Rule

```text
A candidate may enter a fundamental pool only if it is not currently classified as an emergent projection of a deeper mechanism.
```

## Candidate Status Classes

TIG-E uses status classes rather than deletion.

A candidate may be marked as:

- **Fundamental Candidate** — currently treated as potentially generative within the active domain.
- **Candidate** — relevant but not yet classified.
- **Emergent Projection** — useful or observable, but not admissible as fundamental.
- **Rejected** — incompatible with current governance rules or logically invalid.
- **Needs Evidence** — requires empirical, textual, formal, or domain-specific support.
- **Needs Formalization** — conceptually useful but not yet structurally defined.
- **Open Question** — cannot be classified without resolving a dependency.

## Mark, Do Not Delete

Emergent candidates are not erased.

They remain valuable as projections that must eventually be explained by deeper structures.

```text
Emergent structures are evidence of what must be explained, not primitives from which to explain.
```

## Admission Questions

Every candidate must be tested against the following questions:

1. What domain does this candidate belong to?
2. Is it proposed as a generator or a projection?
3. Does it depend on another unresolved structure?
4. Is it domain-specific or domain-general?
5. Does it introduce circular reasoning?
6. Does it rely on concepts that are themselves emergent in the active domain?
7. What evidence or formalization is required before promotion?

## Domain Examples

In a physics domain pack, time, space, mass, metric, curvature, or energy may be marked as emergent projections if the active hypothesis treats them as non-fundamental.

In a pharmaceutical domain pack, molecular candidates, biological pathways, receptor interactions, or clinical endpoints may require a completely different candidate ontology.

Therefore, no universal document may hard-code a domain-specific candidate list as globally fundamental or globally emergent.

## Governance Rule

```text
Fundamentality is always evaluated relative to the active domain model and the current generator/projection map.
```

## Review Requirement

No candidate may be promoted from Candidate or Open Question to Fundamental Candidate without a recorded rationale.

The rationale must include:

- admission reason,
- known dependencies,
- known risks,
- rejected alternatives if applicable,
- and current evidence status.
